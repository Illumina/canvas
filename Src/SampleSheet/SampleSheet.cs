...
namespace SequencingFiles
{
    ...
    public class SampleSheet
    {
        ...
        protected readonly Dictionary<string, int> _sampleColumns = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
        ...
        public string GetProperGenomeFolderPath(string sampleGenomeFolder, string isasConfigGenomeRepository = null)
        {
            if (sampleGenomeFolder == NoGenomeFolder)
            {
                ...
                return NoGenomeFolder;
            }

            string properGenomeFolderPath = NoGenomeFolder;
            string alternativePath;

            // First, check path relative to repository (if repository is set)
            if (isasConfigGenomeRepository != null)
            {
                string checkPath = Path.Combine(isasConfigGenomeRepository, sampleGenomeFolder);
            ...
            if (properGenomeFolderPath == NoGenomeFolder)
            {
        ...
        /// <summary>
        /// parses a modern (non-Casava) sample sheet
        /// </summary>
        ...
        private bool ParseSampleSheetHeader()
        {
            ...
            foreach (string line in _sections["Header"])
            {
                ...
                {
                    if (lineTokens[0].Equals(InvestigatorHeader, StringComparison.OrdinalIgnoreCase))
                        Header.InvestigatorName = lineTokens[1];
                    else if (lineTokens[0].Equals(ExpNameHeader, StringComparison.OrdinalIgnoreCase))
                        Header.ExperimentName = lineTokens[1];
                    else if (lineTokens[0].Equals(DateHeader, StringComparison.OrdinalIgnoreCase))
                        Header.Date = lineTokens[1];
                    else if (lineTokens[0].Equals(WorkflowHeader, StringComparison.OrdinalIgnoreCase))
                        WorkflowType = lineTokens[1].Trim(); // trim leading/trailing whitespace from workflow name
                    else if (lineTokens[0].Equals(ChemistryHeader, StringComparison.OrdinalIgnoreCase))
                        Header.Chemistry = lineTokens[1];
                    else if (lineTokens[0].Equals(ApplicationHeader, StringComparison.OrdinalIgnoreCase))
                        Header.Application = lineTokens[1].Trim(); // trim leading/trailing whitespace from application name
                    else if (lineTokens[0].Equals(AssayHeader, StringComparison.OrdinalIgnoreCase))
        ...
        private bool ParseDataSection(string runFolder, Type sampleType)
        {
            ...
            for (int lineIndex = 1; lineIndex < _sections["Data"].Count; lineIndex++)
            {
                ...
                foreach (int laneNumber in tempLanes)
                {
                    ...
                    string index = string.Format("{0}|{1}", indexSequence1, indexSequence2);

                    // refactored workflows allow multiple indexes for a single sample, so don't fail here
                    if (sample.RunFoldersAndLanes.ContainsKey(key) && sample.RunFoldersAndLanes[key] == index)
                    {
                        throw new Exception(string.Format("Error: Redundant sample {0} for {1}", sample.SampleID, sampleRunFolder));
                    }

                    sample.RunFoldersAndLanes[key] = index;
                }
        ...
        private int GetSettingKeyIndex(string key)
        {
            ...
            for (int index = 0; index < _sections["Settings"].Count; index++)
            {
                ...
                if (line.Length > 0 && line[0].Equals(key, StringComparison.OrdinalIgnoreCase))
                    return index;
            }
        ...
        private void WriteData(StreamWriter writer)
        {
            ...
            if (requireIndex && !keyList.Contains("index", StringComparer.OrdinalIgnoreCase))
                keyList.Add("Index");
            if (requireIndex2 && !keyList.Contains("index2", StringComparer.OrdinalIgnoreCase))
                keyList.Add("Index2");
            if (requireLanes && !keyList.Contains("lane", StringComparer.OrdinalIgnoreCase) && !keyList.Contains("lanes", StringComparer.OrdinalIgnoreCase))
                keyList.Add("Lanes");
            ...
            if (!keyList.Contains("RunFolder", StringComparer.OrdinalIgnoreCase))
        ...
        public class Sample
        {
            ...
            {
                return samples.SelectMany(s => s.RunFolders).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
            }
            ...
            {
                return new Dictionary<string, List<Sample>>(InternalGetSamplesByGenome(samples), StringComparer.OrdinalIgnoreCase);
            }
