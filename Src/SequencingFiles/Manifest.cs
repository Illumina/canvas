...
namespace SequencingFiles
{
    ...
    public class NexteraManifest
    {
        ...
        protected void init(Stream inputStream, string manifestName, HashSet<string> ExcludeRegionGroups, ErrorHandler Error)
        {
                ...
                while (true)
                {
                    ...
                    if (lineNumber != -1)
                    {
                        ...
                        if (lineNumber > 1)
                        {
                            ...
                            if (region.Start + region.StartProbeLength + region.EndProbeLength > region.End)
                            {
                                throw new Exception(string.Format(
            ...
            if (lineNumber == -1)
                throw new Exception(string.Format("Invalid manifest: No [Regions] section seen in {0}", manifestName));
            if (Regions.Count == 0)
                throw new Exception(string.Format("Error: No regions seen in manifest {0}", manifestName));
            ...
            if (ExcludeRegionGroups != null)
            {
                foreach (string ExcludeGroup in ExcludeRegionGroups)
                {
                    ...
                    {
                        if (Error != null) Error(string.Format("Warning: ExcludeRegionGroups setting specified group '{0}', but no regions in manifest file {1} are assigned to this group; no region exclusion performed",
                            ExcludeGroup, manifestName));
                    }
                }
            }
