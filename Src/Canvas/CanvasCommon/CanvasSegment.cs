...
namespace CanvasCommon
{
	...
	public class CanvasSegment
	{
            ...
            List<string> extraHeaders, bool reportPloidy, PloidyInfo ploidy, bool reportAllSites = false, bool reportGermlineGenotype = false)
		{
			...
			using (BgzipOrStreamWriter writer = new BgzipOrStreamWriter(outVcfPath))
			{
				...
				foreach (GenomeMetadata.SequenceMetadata chromosome in genome.Sequences)
				{
					foreach (CanvasSegment segment in segments)
					{
						if (segment.Chromosome.ToLowerInvariant() != chromosome.Name.ToLowerInvariant()) continue;
						int referenceCN = 2;
						...
						writer.Write("{0}\t{1}\tCanvas:{2}:{0}:{3}:{4}\t", segment.Chr, isReferenceCall ? segment.Begin : segment.Begin - 1, cnvtype, segment.Begin, segment.End);
                        ...
                        //  FORMAT field
                        if (reportGermlineGenotype)
                            writer.Write("END={0}\tGT:RC:BC:CN", segment.end);
                        else
                            writer.Write("END={0}\tRC:BC:CN", segment.end);
			...
			using (StreamWriter writer = new StreamWriter(filePath))
			{
				...
				foreach (GenomeMetadata.SequenceMetadata chromosome in genome.Sequences)
				{
					...
					while (pointStartPos < chromosome.Length)
					{
                        ...
                        // Find the most common major chormosome count, for the most common copy number:
						int majorChromosomeCount = 0;
                        bestCount = 0;
                        ...
                        foreach (CanvasSegment segment in overlapSegments)
                        {
                            if ((majorCopyNumber == 2 && segment.copyNumber != 2) ||
                                ...
                                (majorCopyNumber > 2 && segment.copyNumber <= 2)) continue;
		...
		/// </summary>
		public enum QScoreMethod { BinCountLinearFit, GeneralizedLinearFit, Logistic };
		public int ComputeQScore(QScoreMethod qscoreMethod)
		{
			switch (qscoreMethod)
			{
                case QScoreMethod.Logistic:
                    // Logistic model using the old feature-set.  Gives ROC curve area 0.8256
                    //double score = -1.864;
                    //score += GetQScorePredictor(QScorePredictor.LogBinCount) * 0.9237;
                    //score += GetQScorePredictor(QScorePredictor.ModelDistance) * -63.1553;
                    //score += GetQScorePredictor(QScorePredictor.MafMean) * 1.2597;
                    //score += GetQScorePredictor(QScorePredictor.LogMafCv) * 2.3886;
                    //score += GetQScorePredictor(QScorePredictor.MajorChromosomeCount) * 0.0692;
                    ...
                    return (int)(Math.Round(-10 * Math.Log10(1 - score)));
				...
				case QScoreMethod.GeneralizedLinearFit: // Generalized linear fit with linear transformation to QScore
					...
					double qscore = -11.9 - 11.4 * linearFit; // Scaling to achieve 2 <= qscore <= 61
					qscore = Math.Max(2, qscore);
					qscore = Math.Min(61, qscore);
					return (int)Math.Round(qscore, 0, MidpointRounding.AwayFromZero);
                default:
		...
		/// </summary>
		public enum QScorePredictor { BinCount, LogBinCount, BinMean, BinCv, MafCount, MafMean, MafCv, LogMafCv, ModelDistance, 
