...
namespace CanvasPartition
{
    internal static class R
    {
        ...
        public static double dhyper(double x, double r, double b, double n, bool giveLog)
        {
            ...
            if (CanvasCommon.Utilities.EpsilonEqual(n, 0))
                return CanvasCommon.Utilities.EpsilonEqual(x, 0) ? R.R_D__1(giveLog) : R.R_D__0(giveLog);
            double p = n / (r + b);
            double q = (r + b - n) / (r + b);
        ...
        {
            double lf, lc;
            if (CanvasCommon.Utilities.EpsilonEqual(p, 0))
                return CanvasCommon.Utilities.EpsilonEqual(x, 0) ? R.R_D__1(giveLog) : R.R_D__0(giveLog);

            if (CanvasCommon.Utilities.EpsilonEqual(q, 0))
                return CanvasCommon.Utilities.EpsilonEqual(x, n) ? R.R_D__1(giveLog) : R.R_D__0(giveLog);

            if (CanvasCommon.Utilities.EpsilonEqual(x, 0))
            {
                if (CanvasCommon.Utilities.EpsilonEqual(n, 0))
                    return R.R_D__1(giveLog);

                lc = (p < 0.1) ? (-R.bd0(n, n * q) - n * p) : (n * Math.Log(q));
                return R.R_D_exp(lc, giveLog);
            }
            if (CanvasCommon.Utilities.EpsilonEqual(x, n))
            {
        ...
        public static double stirlerr(double n)
        {
            ...
            {
                nn = n * 2;
                if (CanvasCommon.Utilities.EpsilonEqual(nn, (int)nn))
        ...
        public static double lgammafn_sign(double x, int[] sgn)
        {
            ...
            if (CanvasCommon.Utilities.EpsilonEqual(xmax, 0))
            ...
            // Not sure why 2.0 used instaed of 2 when calculating the remainder
            if (x < 0 && CanvasCommon.Utilities.EpsilonEqual(Math.Floor(-x) % 2.0, 0))
            {
                if (sgn != null) sgn[0] = -1;
            }

            if (x <= 0 && CanvasCommon.Utilities.EpsilonEqual(x, Math.Truncate(x)))
            ...
            if (CanvasCommon.Utilities.EpsilonEqual(sinpiy, 0))
            { /* Negative integer argument ===
        ...
        public static double gammafn(double x)
        {
            ...
            // then return NaN.
            if (CanvasCommon.Utilities.EpsilonEqual(x, 0) || x < 0 && CanvasCommon.Utilities.EpsilonEqual(x, (long)x))
            {
            ...
            else
            {
                ...
                if (y <= 50 && CanvasCommon.Utilities.EpsilonEqual(y, (int)y))
                { // compute (n - 1)!
                    value = 1.0;
                ...
                { // normal case
                    value = Math.Exp((y - 0.5) * Math.Log(y) - y + M_LN_SQRT_2PI +
                        (CanvasCommon.Utilities.EpsilonEqual(2 * y, (int)(2 * y)) ? stirlerr(y) : lgammacor(y)));
                }
                if (x > 0)
                    return value;
        ...
        {
            if (!R.R_finite(x) || !R.R_finite(np) || CanvasCommon.Utilities.EpsilonEqual(np, 0.0))
                return Double.NaN;
            if (Math.Abs(x - np) < 0.1 * (x + np))
            {
                ...
                { /* Taylor series */
                    ...
                    if (CanvasCommon.Utilities.EpsilonEqual(s1, s)) /* last term was effectively 0 */
                        return (s1);
                    s = s1;
                }
            }
        ...
        public static double log1p(double x)
        {
            ...
            int nlnrel = 0;
            double xmin = 0.0;
            if (CanvasCommon.Utilities.EpsilonEqual(xmin, 0)) xmin = -1 + Math.Sqrt(DBL_EPSILON);/*was sqrt(d1mach(4)); */
            if (nlnrel == 0) /* initialize chebychev coefficients */
                nlnrel = chebyshev_init(alnrcs, 43, DBL_EPSILON / 20);/*was .1*d1mach(3)*/

            if (CanvasCommon.Utilities.EpsilonEqual(x, 0)) return 0.0; /* speed */
            if (CanvasCommon.Utilities.EpsilonEqual(x, -1)) return Double.NegativeInfinity;
